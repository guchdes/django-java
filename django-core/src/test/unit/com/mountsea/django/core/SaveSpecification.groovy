...
class SaveSpecification extends DaoOperationalSpecification {
    ...
    def "test saveByKey basics"() {
        ...
        def saveResult = DaoOperationalSpecification.dao.saveByKey(document, SaveMode.INSERT_ONLY)
        then:
        saveResult.inserted
        !saveResult.updated
        ...
        DaoOperationalSpecification.dao.saveByKey(document, SaveMode.INSERT_ONLY)
        then:
        ...
        saveResult = DaoOperationalSpecification.dao.saveByKey(document, SaveMode.UPDATE_ONLY)
        then:
        !saveResult.inserted
        saveResult.updated
        documentEquals(document, findDocument0(clazz, key))
        ...
        saveResult = DaoOperationalSpecification.dao.saveByKey(document, SaveMode.INSERT_OR_UPDATE)
        then:
        !saveResult.inserted
        saveResult.updated
        documentEquals(document, findDocument0(clazz, key))
    ...
    def "test saveByFields basics"() {
        ...
        when: 'insert new with INSERT_ONLY'
        def saveResult = DaoOperationalSpecification.dao.saveByFields(document, SaveMode.INSERT_ONLY, fields)
        then:
        saveResult.inserted
        !saveResult.updated
        documentEquals(document, findDocument0(clazz, key))
        ...
        when: 'insert existing document with INSERT_ONLY'
        DaoOperationalSpecification.dao.saveByFields(document, SaveMode.INSERT_ONLY, fields)
        then:
        thrown(MongoException)
        !DocumentNodeHelper.hasEnableUpdateCollect(document)
        ...
        document.setting.number = 666
        document.items[1].tags.add('f')
        saveResult = DaoOperationalSpecification.dao.saveByFields(document, SaveMode.UPDATE_ONLY, fields)
        then:
        !saveResult.inserted
        saveResult.updated
        documentEquals(document, findDocument0(clazz, key))
        ...
        document.setting.note = null
        document.items[1].tags.add('g')
        saveResult = DaoOperationalSpecification.dao.saveByFields(document, SaveMode.INSERT_OR_UPDATE, fields)
        then:
        !saveResult.inserted
        saveResult.updated
        documentEquals(document, findDocument0(clazz, key))
    ...
    def "should throws when document no id and id type is not ObjectId"() {
        ...
        if (byKey) {
            DaoOperationalSpecification.dao.saveByKey(document, saveMode)
        } else {
            DaoOperationalSpecification.dao.saveByFields(document, saveMode, ['name'])
        }
        then:
    ...
    def "should saveByFields throws when fields contains id and fields.size() > 1"() {
        when:
        NameKeyIntIdDocument document = parseDocument(NameKeyIntIdDocument, ['name': 'k1'] + base)
        DaoOperationalSpecification.dao.saveByFields(document, SaveMode.INSERT_ONLY, ['id', 'name'])
        then:
