...
import static KeyDefinitionTestClasses.*
...
class KeyDefinitionSpecification extends DaoOperationalSpecification {
    ...
    def "test key define basics"() {
        ...
        documentEquals(extractBsonKey, definition.getBsonKeyConverter().keyToBsonDocument(key, codecRegistry, false))
        documentEquals(extractBsonKey, definition.getBsonKeyConverter().keyToBsonFilter(key, codecRegistry, false))
    ...
    def "should throws when document key fields contains null"() {
        ...
        definition.bsonKeyConverter.keyToBsonDocument(key, codecRegistry, false)
        then:
        thrown(IllegalKeyDjangoException)
