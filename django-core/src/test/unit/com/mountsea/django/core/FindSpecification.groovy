...
class FindSpecification extends DaoOperationalSpecification {
    ...
    def "test findByKey and projection"() {
        ...
        def document = dao.findByKey(IntIdDocument, 1, Projections.include("setting"))
        then:
    ...
    def "test findOne"() {
        ...
        insertDocument0(clazz, parse)
        def document = dao.findOne(clazz, bsonKey)
        then:
        documentEquals(expectDocument, document)
    ...
    def "test findOne and projection"() {
        when:
        parseAndInsertDocument0(IntIdDocument, ['_id': 1, 'name': 'ss'] + base)
        def document = dao.findOne(IntIdDocument, MultiEquals.withId(1), Projections.include("setting"))
        then:
        documentEquals(['setting': base.get('setting'), '_id': 1], document)
    }
    ...
    def "test findAll"() {
        ...
        when:
        def list = dao.findAll(IntIdDocument, Filters.eq("setting.noticeOn", true))
        then:
        documentSetEquals([d1, d2], list)

        when:
        list = dao.findAll(IntIdDocument, Filters.eq("setting.noticeOn", true), Projections.include("base"))
        then:
        documentSetEquals([['_id': 1, 'base': 'base'], ['_id': 2, 'base': 'base']], list)

        when:
        list = dao.findAll(IntIdDocument, Filters.eq("setting.noticeOn", true), null, Sorts.descending("_id"))
        then:
        documentListEquals([d2, d1], list)

        when:
        list = dao.findAll(IntIdDocument, Filters.eq("setting.noticeOn", true), null,
